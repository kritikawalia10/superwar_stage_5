...
class Superwar {
  ...
  fight = () => {
    ...
    let sol = document.getElementById("score");
  ...
  checkWin = () => {
    ...
    if(healing["hero"]>healing["villain"]){
      result = "hero";
    } 
    else if (healing["hero"]==healing["villain"]) {
      result = "endure"
    }
     else if (healing["hero"]< healing["villain"]){
       result = "villain"
  ...
  topper = ()=>{
    ...
    let topper = this.checkWin();

    if(topper == "hero"){
      win["hero"] += 1;
    } else if(topper=="villain"){
      win["villain"] += 1;
    } else{
      win["draw"] += 1;
    }
